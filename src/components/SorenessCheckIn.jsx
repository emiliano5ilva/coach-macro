...
export function SorenesSummary({ score, muscles }) {
  const c = scoreColor(score) || { bg:'rgba(245,245,240,0.06)', border:'rgba(245,245,240,0.1)', text:'rgba(245,245,240,0.5)' };
  return (
    <div style={{marginTop:12,paddingTop:12,borderTop:'1px solid rgba(232,52,28,0.12)',display:'flex',alignItems:'center',gap:10}}>
      ...
      <div>
        <div style={{...mono,fontSize:9,color:'#f5f5f0',letterSpacing:'0.08em'}}>Soreness logged</div>
        {muscles?.length > 0 && (
          <div style={{...mono,fontSize:9,color:'rgba(245,245,240,0.5)',marginTop:2}}>{muscles.join(', ')}</div>
        )}
      </div>
    </div>
  );
...
export default function SorenessCheckIn({ userId, onComplete, onSkip }) {
  ...
  return (
    <div style={{marginTop:16,paddingTop:16,borderTop:'1px solid rgba(232,52,28,0.12)'}}>
      ...
      <div style={{...cond,fontStyle:'italic',fontWeight:900,fontSize:18,color:'#f5f5f0',lineHeight:0.95,marginBottom:14}}>Rate your soreness today.</div>
        ...
        {Array.from({length:10},(_,i)=>i+1).map(n=>{
          ...
          return (
            <div key={n} onClick={()=>setScore(n)} style={{
              ...
              background: col ? col.bg : 'rgba(245,245,240,0.06)',
              border: `1px solid ${col ? col.border : 'rgba(245,245,240,0.1)'}`,
              color: col ? col.text : 'rgba(245,245,240,0.4)',
      ...
      <div style={{display:'flex',justifyContent:'space-between',...mono,fontSize:7,color:'rgba(245,245,240,0.3)',letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:showMuscles?0:14}}>
      ...
      <div style={{
        ...
        <div style={{...mono,fontSize:9,color:'rgba(245,245,240,0.5)',marginBottom:8,letterSpacing:'0.08em',textTransform:'uppercase'}}>Where are you sore?</div>
          ...
          {MUSCLES.map(m=>{
            ...
            return (
              <div key={m} onClick={()=>toggleMuscle(m)} style={{
                ...
                background: on ? 'rgba(232,52,28,0.15)' : 'rgba(245,245,240,0.05)',
                border: `1px solid ${on ? 'rgba(232,52,28,0.4)' : 'rgba(245,245,240,0.1)'}`,
                color: on ? '#e8341c' : 'rgba(245,245,240,0.4)',
                transition:'all 0.15s',
              }}>{m}</div>
            );
          })}
      ...
      {score > 0 && (
        <div style={{display:'flex',gap:8}}>
          ...
          <button onClick={onSkip} style={{
            flex:1,background:'transparent',border:'1px solid rgba(245,245,240,0.1)',
            borderRadius:10,padding:11,
            ...mono,fontSize:9,color:'rgba(245,245,240,0.4)',letterSpacing:'0.14em',
