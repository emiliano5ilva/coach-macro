...
export default function CoachMacro() {
  ...
  return <App profile={profile} schedule={schedule} setSchedule={setSchedule} dayFocus={dayFocus} wPrefs={wPrefs} setWPrefs={setWPrefs} onEarnedCals={cals=>setEarnedCals(prev=>prev+cals)} onSignOut={handleSignOut} user={user}/>;
      ...
      {/* APP SCREENSHOTS */}
      <section style={{padding:"80px 24px",background:"#080808",overflow:"hidden"}}>
        <div style={{textAlign:"center",marginBottom:56}}>
          ...
          <p style={{fontSize:16,color:"#666",maxWidth:480,margin:"0 auto",lineHeight:1.65}}>Every screen designed around your goal. Not generic. Not one-size-fits-all.</p>
        </div>

        {/* 3 phone mockups in a row */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:24,maxWidth:1000,margin:"0 auto",alignItems:"start"}}>

          {/* SCREEN 1 — Fuel Dashboard */}
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
            <div style={{background:"#0A0A12",border:"1px solid #1C1C2E",borderRadius:32,overflow:"hidden",width:"100%",maxWidth:260,boxShadow:"0 30px 60px rgba(0,0,0,.7)"}}>
              <div style={{background:"#060610",padding:"16px 14px 14px"}}>
                <div style={{width:40,height:4,background:"#1A1A2E",borderRadius:2,margin:"0 auto 14px"}}/>
                {/* Header */}
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                  <div>
                    <div style={{fontSize:9,color:"#444",letterSpacing:1}}>TRAINING DAY</div>
                    <div style={{fontSize:15,fontWeight:800,color:"#fff"}}>Fuel 🔥</div>
                  </div>
                ...
                <div style={{position:"relative",textAlign:"center",marginBottom:14}}>
                  ...
                  <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center"}}>
                    ...
                    <div style={{fontSize:9,color:"#2979FF",marginTop:2}}>2847 budget</div>
                  </div>
                </div>
                {/* Macro bars */}
                {[["Protein","#2979FF","195g / 240g",81],["Carbs","#00E676","186g / 320g",58],["Fat","#FFD740","47g / 68g",69]].map(([n,c,v,p])=>(
                ...
                <div style={{marginTop:12,borderTop:"1px solid #111",paddingTop:10}}>
                  ...
                  {[["Greek Yogurt + Berries","🥣","312 kcal","P:28 C:32 F:6"],["Grilled Chicken Breast","🍗","280 kcal","P:52 C:0 F:6"]].map(([food,e,cal,macros])=>(
                    <div key={food} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,padding:"6px 8px",background:"#0D0D1A",borderRadius:8}}>
                      <div style={{fontSize:14}}>{e}</div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:9,color:"#ccc",fontWeight:600}}>{food}</div>
                        <div style={{fontSize:8,color:"#444"}}>{macros}</div>
                      </div>
            ...
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:4}}>Fuel Dashboard</div>
              <div style={{fontSize:12,color:"#666"}}>Dynamic macros · AI food logging · Restaurant finder</div>
            </div>
          </div>

          {/* SCREEN 2 — Train Today + Muscle Map */}
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
            <div style={{background:"#0A0A12",border:"1px solid #1C1C2E",borderRadius:32,overflow:"hidden",width:"100%",maxWidth:260,boxShadow:"0 30px 60px rgba(0,0,0,.7)"}}>
              <div style={{background:"#060610",padding:"16px 14px 14px"}}>
                <div style={{width:40,height:4,background:"#1A1A2E",borderRadius:2,margin:"0 auto 14px"}}/>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  <div>
                    <div style={{fontSize:9,color:"#444",letterSpacing:1}}>MONDAY</div>
                    <div style={{fontSize:15,fontWeight:800,color:"#fff"}}>Train 💪</div>
                  </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:12}}>
                  <div style={{background:"#0D0D1A",borderRadius:10,padding:8}}>
                    <div style={{fontSize:8,color:"#444",marginBottom:4}}>TODAY'S FOCUS</div>
                    {[["Chest","#E8185A","Optimal"],["Shoulders","#E8185A","Optimal"],["Triceps","#8B1A6B","Building"]].map(([m,c,s])=>(
                      <div key={m} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                        <span style={{fontSize:8,color:"#ccc"}}>{m}</span>
                        <span style={{fontSize:7,color:c,background:`${c}15`,borderRadius:4,padding:"1px 5px"}}>{s}</span>
                      </div>
                    ))}
                  </div>
                  {/* Mini SVG body */}
                  <div style={{background:"#0D0D1A",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",padding:4}}>
                    <svg viewBox="0 0 100 160" width="60" height="96">
                      ...
                      {/* Biceps - dark */}
                      <path d="M18,44 Q10,58 12,72" fill="none" stroke="#1A1A2E" strokeWidth="5" strokeLinecap="round"/>
                      <path d="M82,44 Q90,58 88,72" fill="none" stroke="#1A1A2E" strokeWidth="5" strokeLinecap="round"/>
                    </svg>
                  </div>
                </div>
                {/* Active session */}
                <div style={{background:"#0D0D1A",borderRadius:10,padding:10,marginBottom:8}}>
                  <div style={{fontSize:8,color:"#444",marginBottom:8}}>CURRENT SESSION</div>
                  {[["Barbell Bench Press","4×8","185 lbs","↑ +5 from last"],["Incline DB Press","3×10","65 lbs","PR ⚡"],["Cable Fly","3×12","40 lbs",""]].map(([ex,sets,w,note])=>(
                    <div key={ex} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,paddingBottom:6,borderBottom:"1px solid #111"}}>
                      <div>
                        <div style={{fontSize:8,color:"#ccc",fontWeight:600}}>{ex}</div>
                        <div style={{fontSize:7,color:"#00C9A7"}}>{note||sets}</div>
                      </div>
                      <div style={{textAlign:"right"}}>
                        <div style={{fontSize:9,fontWeight:700,color:"#fff"}}>{w}</div>
                        <div style={{fontSize:7,color:"#444"}}>{sets}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:4}}>Train Dashboard</div>
              <div style={{fontSize:12,color:"#666"}}>Muscle map · Progressive overload · Auto rest timer</div>
            </div>
          </div>

          {/* SCREEN 3 — Program / Progress */}
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
            <div style={{background:"#0A0A12",border:"1px solid #1C1C2E",borderRadius:32,overflow:"hidden",width:"100%",maxWidth:260,boxShadow:"0 30px 60px rgba(0,0,0,.7)"}}>
              <div style={{background:"#060610",padding:"16px 14px 14px"}}>
                <div style={{width:40,height:4,background:"#1A1A2E",borderRadius:2,margin:"0 auto 14px"}}/>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  <div>
                    <div style={{fontSize:9,color:"#444",letterSpacing:1}}>WEEK 4 OF 12</div>
                    <div style={{fontSize:15,fontWeight:800,color:"#fff"}}>Progress 📈</div>
                  </div>
                ...
                <div style={{background:"#0D0D1A",borderRadius:10,padding:10,marginBottom:10}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <div style={{fontSize:9,color:"#888"}}>Push/Pull/Legs · 6-day</div>
                    <div style={{fontSize:9,color:"#FFD740",fontWeight:700}}>33%</div>
                  </div>
                  <div style={{height:5,background:"#111",borderRadius:3,marginBottom:8}}>
                    <div style={{height:"100%",width:"33%",background:"linear-gradient(90deg,#2979FF,#FFD740)",borderRadius:3}}/>
                  </div>
                ...
                {/* PR tracker */}
                <div style={{background:"#0D0D1A",borderRadius:10,padding:10,marginBottom:10}}>
                  <div style={{fontSize:8,color:"#444",marginBottom:6}}>PERSONAL RECORDS</div>
                  {[["Bench Press","225 lbs","↑ +20 this program"],["Squat","315 lbs","↑ +35 this program"],["Deadlift","365 lbs","↑ +40 this program"]].map(([lift,w,note])=>(
                    <div key={lift} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                      <div>
                        <div style={{fontSize:8,color:"#ccc"}}>{lift}</div>
                        <div style={{fontSize:7,color:"#00E676"}}>{note}</div>
                      </div>
                      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:16,fontWeight:900,color:"#fff"}}>{w}</div>
                    </div>
                  ))}
                </div>
                {/* Weight trend */}
                <div style={{background:"#0D0D1A",borderRadius:10,padding:10}}>
                  ...
                  <svg width="100%" height="50" viewBox="0 0 220 50">
                    {/* Projected line */}
                    <path d="M0,40 L55,34 L110,28 L165,22 L220,16" fill="none" stroke="#2979FF" strokeWidth="1.5" strokeDasharray="4,3" opacity="0.4"/>
                    {/* Actual line */}
                    <path d="M0,40 L55,36 L110,31 L165,27" fill="none" stroke="#00E676" strokeWidth="2" strokeLinecap="round"/>
                    {/* Actual dots */}
                    {[[0,40],[55,36],[110,31],[165,27]].map(([x,y],i)=>(
                      <circle key={i} cx={x} cy={y} r="3" fill="#00E676"/>
                    ))}
                    {/* Labels */}
                    <text x="0" y="48" fill="#444" fontSize="7">Start</text>
                    <text x="165" y="48" fill="#00E676" fontSize="7">Today</text>
                  </svg>
            ...
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:4}}>Progress Tracking</div>
              <div style={{fontSize:12,color:"#666"}}>Program progress · PR tracker · Weight trend</div>
            </div>
          </div>
        ...
        <div style={{textAlign:"center",marginTop:56}}>
          <button onClick={onSignUp} style={{background:"#2979FF",color:"#fff",border:"none",borderRadius:9,padding:"16px 36px",fontWeight:700,fontSize:16,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>Start Your Free Trial →</button>
        </div>
      </section>
