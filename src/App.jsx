...
function App({profile,schedule,setSchedule,dayFocus,wPrefs,setWPrefs,onEarnedCals,onSignOut,user}) {
  ...
  async function startStructured(){
    try{
      const splitInfo=split?`Split: ${split}.`:"";
      const runInfo=runPlan?`Run plan: ${runPlan}.`:"";
      const hybridInfo=hybridTemplate?`Hybrid template: ${hybridTemplate}.`:"";
      const raw=await ai(`Structured ${todayFocus} workout for ${profile.goal} athlete.\nEquipment: ${wPrefs.equipment}. ${splitInfo}${runInfo}${hybridInfo}\nFocus: ${FOCUS_MUSCLES[todayFocus]||"full body"}.\nReturn ONLY valid JSON no markdown:\n{"exercises":[{"name":"Exercise","sets":[{"reps":10,"weight":0,"done":false}],"restSecs":90,"notes":"form cue — which muscle head this targets"}]}\n5-7 exercises. Compounds 3-4 sets. Isolation 2-3 sets. restSecs: 180 heavy compounds, 90 secondary, 60 isolation. Cover ALL muscle heads listed in focus.`,700);
...
function WorkoutBuilder({profile,wPrefs,setWPrefs,generateWorkout,startStructured,workout,workoutLoading,isMobile,todayFocus}) {
...
function LandingPage({onSignUp}) {
  ...
  return(
    <div style={{minHeight:"100vh",background:"#080808",color:"#fff",fontFamily:"'Inter',system-ui,sans-serif",overflowX:"hidden",width:"100%",maxWidth:"100vw"}}>
      ...
      {/* HERO */}
      <section style={{minHeight:"100vh",padding:"120px 24px 60px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:40,alignItems:"center",maxWidth:1200,margin:"0 auto"}}>
        <div>
        ...
        <div className="hero-phone" style={{justifyContent:"center",alignItems:"center"}}>
...
export default function CoachMacro() {
  ...
  async function saveProfile(uid,prof,sch,wp) {
    ...
    try {
      const {data,error}=await sb.from("profiles").upsert({id:uid,profile_data:prof,schedule:sch,wprefs:wp,updated_at:new Date().toISOString()},{onConflict:"id"}).select();
